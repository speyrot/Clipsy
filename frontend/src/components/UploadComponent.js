    ...
    <div className="w-full space-y-4">
      <form onSubmit={handleUpload} className="w-full space-y-4">
          ...
          <label className="w-full h-32 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 flex flex-col justify-center items-center transition-colors">
            {selectedFile ? (
              ...
              <div className="flex flex-col items-center text-gray-500 text-sm">
                ...
                <span className="text-sm">Click or Drag a Video File Here</span>
              </div>
            )}
            <input
              type="file"
        ...
        <button
          ...
          className={`w-full py-2 text-white rounded-lg text-sm font-semibold transition-colors ${
      ...
      {error && (
        <div className="w-full p-2 bg-red-50 text-red-700 text-sm rounded-md">
          {error}
        </div>
      )}

      {successMessage && (
        <div className="w-full p-2 bg-green-50 text-green-700 text-sm rounded-md">
          {successMessage}
        </div>
      )}
